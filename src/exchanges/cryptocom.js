 ...
 * 
 * Handles interactions with the Crypto.com exchange API for:
...
class CryptocomExchange {
  ...
  async initialize() {
    try {
      this.logger.info('Initializing Crypto.com exchange connector');
      ...
      if (this.apiKey && this.apiSecret) {
        try {
          ...
          this.logger.info('Crypto.com API key valid with trading permissions');
        } catch (error) {
          this.logger.error('Crypto.com API key error:', error.message);
          throw new Error('Invalid API key or insufficient permissions');
        }
      }
      
      this.logger.info('Crypto.com exchange connector initialized successfully');
      
      return true;
    } catch (error) {
      this.logger.error('Failed to initialize Crypto.com exchange connector', error);
      throw error;
    }
  }
  ...
  async updateMarkets() {
    try {
      ...
      this.logger.info(`Loaded ${Object.keys(this.markets).length} markets from Crypto.com`);
      return this.markets;
    } catch (error) {
    ...
    try {
      ...
      if (!this.markets || !this.markets[symbol]) {
        ...
        if (!this.markets[symbol]) {
          throw new Error(`Symbol ${symbol} not found on Crypto.com`);
        }
      }
   ...
   * Includes 24h volume, high, low, etc.
   */
